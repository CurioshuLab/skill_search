...
const targetCount = Number(process.env.SKILL_PACK_TARGET || 1000);
const perPage = 100;
const queries = [
  "ai skills",
  "agent skills",
...
const headers = {
  ...
  "User-Agent": "MENgrandpa-ai-skill-catalog",
...
const dataset = {
  ...
  sourceQueries,
  records: records.slice(0, targetCount)
};

await mkdir(dirname(outputPath), { recursive: true });
await writeFile(outputPath, `${JSON.stringify(dataset, null, 2)}\n`, "utf8");
console.log(`Wrote ${dataset.count} records to ${outputPath}`);
console.log(`Sources: ${sourceQueries.map((entry) => `${entry.query} p${entry.page}`).join(", ")}`);

function toCatalogRecord(item, matchedQuery) {
