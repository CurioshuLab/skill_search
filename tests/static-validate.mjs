...
assert(source.includes('ai-skill-packs.json?url'), "catalog data must be loaded as a static asset URL");
assert(source.includes('fetch(catalogUrl)'), "catalog data must be fetched separately from the JS bundle");
assert(!source.includes('import catalog from "./data/ai-skill-packs.json"'), "catalog data must not be bundled directly into main JavaScript");
...
for (const ignored of ["node_modules", "dist", "output", ".env", ".env.*", ".git", ".github", "tests"]) {
